...
const ARCHIVOS_ESENCIALES = [
  ...
  'uentos-que-no-son-cuentos/No-no-no-y-todo-cambio/audio/pagina-14.mp3',
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(async cache => {
      ...
      // 1. Guardamos el esqueleto (HTML, CSS, JS e imágenes) de golpe
      await Promise.all(
        otrosArchivos.map(url => 
          cache.add(url).catch(e => console.warn('No se pudo cachear recurso básico:', url))
        )
      );

      // 2. Descargamos los audios uno por uno en fila para no saturar al navegador móvil
      for (const url of audioArchivos) {
        try {
          ...
          console.warn('Fallo la descarga inicial del audio:', url);
        }
      }
    })
  );
...
// Fetch: Prioridad Caché para contenido ya instalado (Ideal para modo Offline completo)
self.addEventListener('fetch', event => {
  ...
  // Estrategia para el resto de archivos: busca en caché primero, si no, va a la red
  event.respondWith(
