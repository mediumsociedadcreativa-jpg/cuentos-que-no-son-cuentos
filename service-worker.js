const CACHE_NAME = 'cuentos-v16'; // Subimos de versión para forzar actualización completa en pruebas
    ...
    caches.match(event.request).then(cachedResponse => {
      ...
      return fetch(event.request).then(networkResponse => {
        ...
        if (event.request.mode === 'navigate') {
          if (event.request.mode === 'navigate') {
              return caches.match('/cuentos-que-no-son-cuentos/offline.html');
          } // 👈 Muestra el aviso limpio
        }
      });
    })
  );
